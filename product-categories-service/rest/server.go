...
import (
	...
	"net/http"
	"product-categories-service/api"
	"product-categories-service/config"
	"product-categories-service/rest/middlewares"
...
func NewRestServer(errChan chan error) (*manners.GracefulServer, error) {
	...
	err := category.RegisterProductCategoryServiceHandlerFromEndpoint(ctx, mux, cfg.Server.RPCAddr, opts)
	if err != nil {
