...
func Log(level, message string) {
	logMsg := LogMessage{
		ServiceName: "product-category-service",
		Level:       level,
		Message:     message,
	}
...
func LogFileInit() error {
	appDir := "../logs"
	serviceDir := "product-category-service"
	...
	logFileName := fmt.Sprintf("product-category-service-%s.log", time.Now().Format("2006-01-02"))
