...
func UnaryAuthHandler(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (resp interface{}, err error) {
	...
	if info.FullMethod == "/product.ProductCatalogService/SearchProduct" {
		authorized, errMsg := processAuthorization(claims, "client_id", "product_read_all")
		if !authorized {
			utils.Log("ERROR", errMsg)
			return nil, fmt.Errorf(errMsg)
		}
	} else if info.FullMethod == "/product.ProductCatalogService/CreateProduct" {
		authorized, errMsg := processAuthorization(claims, "client_id", "product_create")
		if !authorized {
			utils.Log("ERROR", errMsg)
			return nil, fmt.Errorf(errMsg)
		}
	} else if info.FullMethod == "/product.ProductCatalogService/GetProductById" {
		authorized, errMsg := processAuthorization(claims, "client_id", "product_detail")
		if !authorized {
			utils.Log("ERROR", errMsg)
			return nil, fmt.Errorf(errMsg)
		}
